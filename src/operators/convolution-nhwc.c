...
static enum xnn_status create_input_T_gemm_or_igemm(
  enum xnn_microkernel_type ukernel_type,
  uint32_t kernel_size,
  uint32_t groups,
  size_t group_input_channels,
  ...
  size_t* zero_size)
{
  enum xnn_status status = xnn_status_out_of_memory;
  const uint32_t nr = gemm_config->nr;
  // in rvv, kr, sr are set to 1
  const uint32_t kr = UINT32_C(1);
  const uint32_t sr = UINT32_C(1);
  const size_t n_stride = round_up(group_output_channels, nr);
  const size_t k_stride = round_up_po2(group_input_channels, kr * sr);
  ...
  assert(XNN_MAX_MR >= mr);
  for (size_t i = 0; i < mr; i++) {
    convolution_op->ukernel.input_T_gemm.gemm_cases[i] = gemm_ukernels->input_T_gemm[i];
  }
...
static enum xnn_status create_input_T_pruned_gemm_or_igemm_x1v(
  enum xnn_microkernel_type ukernel_type,
  uint32_t kernel_size,
  uint32_t groups,
  size_t group_input_channels,
  ...
  const void* gemm_params,
  size_t gemm_params_size,
  const struct xnn_input_T_pruned_x1v_gemm_config* gemm_config,
  bool linear_activation,
  bool relu_activation,
  enum xnn_operator_type operator_type,
  xnn_operator_t convolution_op,
  size_t* zero_size)
{
  ...
  const struct gemm_fused_ukernels* gemm_ukernels = &gemm_config->minmax;
  const uint32_t mr = gemm_config->mr;
  if (linear_activation && gemm_config->linear.input_T_pruned_gemm[mr - 1].function[XNN_UARCH_DEFAULT] != NULL) {
    gemm_ukernels = &gemm_config->linear;
  } else if (relu_activation && gemm_config->relu.input_T_pruned_gemm[mr - 1].function[XNN_UARCH_DEFAULT] != NULL) {
    gemm_ukernels = &gemm_config->relu;
  }
  convolution_op->ukernel.input_T_pruned_gemm = (struct xnn_ukernel_input_T_pruned_gemm) {
    .mr = mr,
    .nr = nr,
    .kr = kr,
    .sr = sr,
  ...
  assert(XNN_MAX_MR >= mr);
  for (size_t i = 0; i < mr; i++) {
    convolution_op->ukernel.input_T_pruned_gemm.gemm_cases[i] = gemm_ukernels->input_T_pruned_gemm[i];
  }
...
static enum xnn_status create_input_T_pruned_gemm_or_igemm_x2v(
  enum xnn_microkernel_type ukernel_type,
  uint32_t kernel_size,
  uint32_t groups,
  size_t group_input_channels,
  ...
  const void* gemm_params,
  size_t gemm_params_size,
  const struct xnn_input_T_pruned_x2v_gemm_config* gemm_config,
  bool linear_activation,
  bool relu_activation,
  enum xnn_operator_type operator_type,
  xnn_operator_t convolution_op,
...
static enum xnn_status create_input_T_pruned_gemm_or_igemm_x4v(
  enum xnn_microkernel_type ukernel_type,
  uint32_t kernel_size,
  uint32_t groups,
  size_t group_input_channels,
  ...
  const void* gemm_params,
  size_t gemm_params_size,
  const struct xnn_input_T_pruned_x4v_gemm_config* gemm_config,
  bool linear_activation,
  bool relu_activation,
  enum xnn_operator_type operator_type,
  xnn_operator_t convolution_op,
...
static enum xnn_status create_input_T_pruned_gemm_or_igemm_x8v(
  enum xnn_microkernel_type ukernel_type,
  uint32_t kernel_size,
  uint32_t groups,
  size_t group_input_channels,
  ...
  const void* gemm_params,
  size_t gemm_params_size,
  const struct xnn_input_T_pruned_x8v_gemm_config* gemm_config,
  bool linear_activation,
  bool relu_activation,
  enum xnn_operator_type operator_type,
  xnn_operator_t convolution_op,
...
static enum xnn_status create_input_T_convolution2d_nhwc(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
  ...
  xnn_operator_t* convolution_op_out)
{
  ...
  status = create_input_T_gemm_or_igemm(
      ...
      kernel, bias, flags,
      log2_input_element_size, log2_filter_element_size, bias_element_size,
      packa_gemm_x4v, pack_conv_kgo_w, pack_conv_goki_w, packing_params,
      packed_weights_padding_byte, extra_weights_bytes,
      gemm_params, gemm_params_size, gemm_config,
      linear_activation, relu_activation,
      operator_type,
...
static enum xnn_status create_input_T_pruned_convolution2d_nhwc_x1v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
  ...
  xnn_operator_t* convolution_op_out)
{
  ...
  const bool unit_subsampling = (subsampling_width | subsampling_height) == 1;
  if (kernel_size == 1 && unit_subsampling && !any_padding) {
    ukernel_type = xnn_microkernel_type_input_T_pruned_gemm;
  } else {
    ukernel_type = xnn_microkernel_type_input_T_pruned_igemm;
  }
  assert(ukernel_type != xnn_microkernel_type_default);

  size_t zero_size = 0;
  status = create_input_T_pruned_gemm_or_igemm_x1v(
      ukernel_type, kernel_size,
      groups, group_input_channels, group_output_channels,
      kernel, bias, flags,
      log2_input_element_size, log2_filter_element_size, bias_element_size,
  ...
  convolution_op->kernel = kernel;
  convolution_op->bias = bias;
  convolution_op->riscv_packa_method = gemm_config->packa_gemm_x1v;
  int nr = gemm_config->nr;
...
static enum xnn_status create_input_T_pruned_convolution2d_nhwc_x2v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
  ...
  xnn_operator_t* convolution_op_out)
{
  ...
  size_t zero_size = 0;
  status = create_input_T_pruned_gemm_or_igemm_x2v(
      ukernel_type, kernel_size,
      groups, group_input_channels, group_output_channels,
      kernel, bias, flags,
      log2_input_element_size, log2_filter_element_size, bias_element_size,
  ...
  convolution_op->kernel = kernel;
  convolution_op->bias = bias;
  convolution_op->riscv_packa_method = gemm_config->packa_gemm_x2v;
  int nr = gemm_config->nr;
...
static enum xnn_status create_input_T_pruned_convolution2d_nhwc_x4v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
  ...
  xnn_operator_t* convolution_op_out)
{
  ...
  size_t zero_size = 0;
  status = create_input_T_pruned_gemm_or_igemm_x4v(
      ukernel_type, kernel_size,
      groups, group_input_channels, group_output_channels,
      kernel, bias, flags,
      log2_input_element_size, log2_filter_element_size, bias_element_size,
...
static enum xnn_status create_input_T_pruned_convolution2d_nhwc_x8v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
  ...
  xnn_operator_t* convolution_op_out)
{
  ...
  size_t zero_size = 0;
  status = create_input_T_pruned_gemm_or_igemm_x8v(
      ukernel_type, kernel_size,
      groups, group_input_channels, group_output_channels,
      kernel, bias, flags,
      log2_input_element_size, log2_filter_element_size, bias_element_size,
  ...
  convolution_op->kernel = kernel;
  convolution_op->bias = bias;
  convolution_op->riscv_packa_method = gemm_config->packa_gemm_x8v;
  int nr = gemm_config->nr;
...
const struct xnn_input_T_gemm_config* gemm_config = xnn_init_f32_input_T_gemm_config();
if (gemm_config == NULL) {
  xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                xnn_operator_type_to_string(xnn_operator_type_convolution_nhwc_f32));
  return xnn_status_unsupported_hardware;
...
return create_input_T_convolution2d_nhwc(
  ...
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*bias_element_size=*/sizeof(float),
  (xnn_x32_packa_gemm_ukernel_fn) gemm_config->packa_gemm_x4v,
  (xnn_pack_conv_kgo_w_fn) xnn_pack_f32_conv_kgo_w,
  (xnn_pack_conv_goki_w_fn) xnn_pack_f32_conv_goki_w,
  /*packing_params=*/NULL,
  /*input_padding_byte=*/0,
...
enum xnn_status xnn_create_input_T_pruned_convolution2d_nhwc_f32_x1v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
...
const struct xnn_input_T_pruned_x1v_gemm_config* gemm_config = xnn_init_f32_input_T_x1v_gemm_config();
if (gemm_config == NULL) {
  xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                xnn_operator_type_to_string(xnn_operator_type_convolution_nhwc_f32));
  return xnn_status_unsupported_hardware;
...
return create_input_T_pruned_convolution2d_nhwc_x1v(
  input_padding_top, input_padding_right, input_padding_bottom, input_padding_left,
  kernel_height, kernel_width,
  subsampling_height, subsampling_width,
  dilation_height, dilation_width,
  ...
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*bias_element_size=*/sizeof(float),
  (xnn_x32_packa_gemm_ukernel_fn) gemm_config->packa_gemm_x1v,
  /*gemm_params=*/&gemm_params,
  /*gemm_params_size=*/sizeof(gemm_params),
  /*gemm_config=*/gemm_config,
  /*linear_activation=*/linear_activation,
...
enum xnn_status xnn_create_input_T_pruned_convolution2d_nhwc_f32_x2v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
...
const struct xnn_input_T_pruned_x2v_gemm_config* gemm_config = xnn_init_f32_input_T_x2v_gemm_config();
if (gemm_config == NULL) {
  xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                xnn_operator_type_to_string(xnn_operator_type_convolution_nhwc_f32));
  return xnn_status_unsupported_hardware;
...
return create_input_T_pruned_convolution2d_nhwc_x2v(
  input_padding_top, input_padding_right, input_padding_bottom, input_padding_left,
  kernel_height, kernel_width,
  subsampling_height, subsampling_width,
  dilation_height, dilation_width,
  ...
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*bias_element_size=*/sizeof(float),
  (xnn_x32_packa_gemm_ukernel_fn) gemm_config->packa_gemm_x2v,
  /*gemm_params=*/&gemm_params,
  /*gemm_params_size=*/sizeof(gemm_params),
  /*gemm_config=*/gemm_config,
  /*linear_activation=*/linear_activation,
...
enum xnn_status xnn_create_input_T_pruned_convolution2d_nhwc_f32_x4v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
...
const struct xnn_input_T_pruned_x4v_gemm_config* gemm_config = xnn_init_f32_input_T_x4v_gemm_config();
if (gemm_config == NULL) {
  xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                xnn_operator_type_to_string(xnn_operator_type_convolution_nhwc_f32));
  return xnn_status_unsupported_hardware;
...
return create_input_T_pruned_convolution2d_nhwc_x4v(
  input_padding_top, input_padding_right, input_padding_bottom, input_padding_left,
  kernel_height, kernel_width,
  subsampling_height, subsampling_width,
  dilation_height, dilation_width,
...
enum xnn_status xnn_create_input_T_pruned_convolution2d_nhwc_f32_x8v(
  uint32_t input_padding_top,
  uint32_t input_padding_right,
  uint32_t input_padding_bottom,
  uint32_t input_padding_left,
...
const struct xnn_input_T_pruned_x8v_gemm_config* gemm_config = xnn_init_f32_input_T_x8v_gemm_config();
if (gemm_config == NULL) {
  xnn_log_error("failed to create %s operator: unsupported hardware configuration",
                xnn_operator_type_to_string(xnn_operator_type_convolution_nhwc_f32));
  return xnn_status_unsupported_hardware;
...
return create_input_T_pruned_convolution2d_nhwc_x8v(
  input_padding_top, input_padding_right, input_padding_bottom, input_padding_left,
  kernel_height, kernel_width,
  subsampling_height, subsampling_width,
  dilation_height, dilation_width,
  ...
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*bias_element_size=*/sizeof(float),
  (xnn_x32_packa_gemm_ukernel_fn) gemm_config->packa_gemm_x8v,
  /*gemm_params=*/&gemm_params,
  /*gemm_params_size=*/sizeof(gemm_params),
  /*gemm_config=*/gemm_config,
  /*linear_activation=*/linear_activation,
...
static enum xnn_status reshape_input_T_gemm(
  xnn_operator_t convolution_op,
  uint32_t log2_input_element_size,
  uint32_t log2_filter_element_size,
  uint32_t extra_weights_elements_size,
  ...
  size_t num_threads)
{
  ...
  uint32_t mr = convolution_op->ukernel.input_T_gemm.mr;
  ...
  #if XNN_ENABLE_GEMM_M_SPECIALIZATION
    mr = xnn_get_heuristic_mr_input_T_gemm(batch_output_size, mr, nr, gemm_cases);
  #else
    if (batch_output_size == 1 && gemm_cases[0].function[XNN_UARCH_DEFAULT] != NULL) {
      mr = 1;
    }
  ...
  struct xnn_hmp_input_T_gemm_ukernel gemm_ukernel = gemm_cases[mr - 1];
...
static enum xnn_status reshape_input_T_pruned_gemm(
  xnn_operator_t convolution_op,
  uint32_t log2_input_element_size,
  uint32_t log2_filter_element_size,
  uint32_t extra_weights_elements_size,
  ...
  const float pruning_ratio)
{
  ...
  uint32_t mr = convolution_op->ukernel.input_T_pruned_gemm.mr;
  ...
  #if XNN_ENABLE_GEMM_M_SPECIALIZATION
    mr = xnn_get_heuristic_mr_input_T_pruned_gemm(batch_output_size, mr, nr, gemm_cases);
  #else
    if (batch_output_size == 1 && gemm_cases[0].function[XNN_UARCH_DEFAULT] != NULL) {
      mr = 1;
    }
  ...
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm = (struct input_T_pruned_gemm_context){
      .k_scaled = pruned_group_input_channels << log2_input_element_size,
      .a_stride = convolution_op->input_pixel_stride << log2_input_element_size,
      .ga_stride = pruned_group_input_channels << log2_input_element_size,
      .w_stride = w_stride,
      .gw_stride = w_stride * round_up(group_output_channels, nr),
      .cm_stride = batch_output_size << log2_output_element_size,
      .cn_stride = nr << log2_output_element_size,
      ...
      .num_batch_dims = 1,
      .ukernel = gemm_ukernel,
      .indice_stride = pruned_group_input_channels << XNN_LOG2_SIZEOF_UINT16_T,
      .mr = mr
  };
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.kernel = convolution_op->kernel;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.bias = convolution_op->bias;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.packa_method = convolution_op->riscv_packa_method;
  xnn_log_debug("kernel address: %p", convolution_op->kernel);
  xnn_log_debug("bias address: %p", convolution_op->bias);
  xnn_log_debug("pruned_group_input_channels: %d", pruned_group_input_channels);
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.batch_dims_a[0] = groups;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.batch_dims_b[0] = groups;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.batch_strides_c[0] = 1;
  ...
  if (groups == 1) {
    convolution_op->compute[0].type = xnn_parallelization_type_2d_tile_2d;
    convolution_op->compute[0].task_2d_tile_2d = (pthreadpool_task_2d_tile_2d_t) xnn_compute_input_T_pruned_gemm;
    convolution_op->compute[0].range[0] = group_output_channels;
    convolution_op->compute[0].range[1] = batch_output_size;
    convolution_op->compute[0].tile[0] = mr;
    convolution_op->compute[0].tile[1] = nc;
...
// gemm with im2col
static enum xnn_status reshape_input_T_igemm(
  xnn_operator_t convolution_op,
  uint32_t log2_input_element_size,
  uint32_t log2_filter_element_size,
  uint32_t extra_weights_elements_size,
...
static enum xnn_status reshape_input_T_pruned_igemm(
  xnn_operator_t convolution_op,
  uint32_t log2_input_element_size,
  uint32_t log2_filter_element_size,
  uint32_t extra_weights_elements_size,
  ...
  const float pruning_ratio)
{
  ...
  *workspace_alignment = 1;
  ...
  const size_t group_output_channels = convolution_op->group_output_channels;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm = (struct input_T_pruned_gemm_context){
      .k_scaled = pruned_im2col_col_cnt << log2_input_element_size,
      .a_stride = group_input_channels * kernel_size << log2_input_element_size,
      .ga_stride = pruned_im2col_col_cnt << log2_input_element_size,
      .w_stride = w_stride,
      .gw_stride = w_stride * round_up(group_output_channels, nr),
      .cm_stride = batch_output_size << log2_output_element_size,
      .cn_stride = nr << log2_output_element_size,
      ...
      .num_batch_dims = 1,
      .ukernel = gemm_ukernel,
      .indice_stride = pruned_im2col_col_cnt << XNN_LOG2_SIZEOF_UINT16_T,
      .mr = mr
  };
  memcpy(&convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.params, &convolution_op->params, sizeof(convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.params));
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.fused_params = &convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.params;
...
static enum xnn_status reshape_input_T_pruned_convolution2d_nhwc(
  xnn_operator_t convolution_op,
  enum xnn_operator_type expected_operator_type,
  size_t batch_size,
  size_t input_height,
...
enum xnn_status xnn_reshape_input_T_convolution2d_nhwc_f32(
  xnn_operator_t convolution_op,
  size_t batch_size,
  size_t input_height,
  size_t input_width,
  ...
  pthreadpool_t threadpool)
{
return reshape_input_T_convolution2d_nhwc(
  convolution_op, xnn_operator_type_convolution_nhwc_f32,
  batch_size, input_height, input_width,
  /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
...
enum xnn_status xnn_reshape_input_T_pruned_convolution2d_nhwc_f32(
  xnn_operator_t convolution_op,
  size_t batch_size,
  size_t input_height,
  size_t input_width,
  ...
  float pruning_ratio)
{
return reshape_input_T_pruned_convolution2d_nhwc(
  convolution_op, xnn_operator_type_convolution_nhwc_f32,
  batch_size, input_height, input_width,
  /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
  /*log2_filter_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
...
static enum xnn_status setup_input_T_gemm(xnn_operator_t convolution_op, uint32_t log2_input_element_size)
{
  const size_t batch_size = convolution_op->batch_size;
  ...
  const size_t batch_output_size = batch_size * output_size;
  const uint32_t nr = convolution_op->ukernel.input_T_gemm.nr;
  const size_t n_stride = round_up(batch_output_size, nr);
  const size_t group = convolution_op->groups;
  const size_t group_input_channels = convolution_op->group_input_channels;
  const size_t packed_group_input_T_size =
      (group_input_channels << log2_input_element_size) * n_stride;
  ...
  convolution_op->context.input_T_gemm.input_T_gemm.gemm.packa_method(
    group, group_input_channels, batch_output_size, nr, 1, 1, 
    convolution_op->input, NULL, input_packed_ptr, 0, NULL
  );
...
static enum xnn_status setup_input_T_pruned_gemm(xnn_operator_t convolution_op, uint32_t log2_input_element_size)
{
  const size_t batch_size = convolution_op->batch_size;
  ...
  const size_t aligned_total_input_T_size = round_up_po2(packed_group_input_T_size * group, XNN_ALLOCATION_ALIGNMENT);
  void* input_packed_ptr = xnn_allocate_simd_memory(aligned_total_input_T_size);
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.packa_method(
    group, group_input_channels, batch_output_size, nr, 1, 1, 
    convolution_op->input, NULL, input_packed_ptr, 0, NULL
  );
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.packed_a = input_packed_ptr;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.c = convolution_op->output;
  convolution_op->context.input_T_gemm.input_T_gemm.pruned_gemm.indice = convolution_op->indice;
  convolution_op->state = xnn_run_state_ready;
...
static enum xnn_status setup_input_T_igemm(
  xnn_operator_t convolution_op,
  void* workspace,
  uint32_t log2_input_element_size,
  xnn_x32_packa_with_im2col_gemm_ukernel_fn im2col_packa)
{
  const size_t batch_size = convolution_op->batch_size;
  ...
  if(convolution_op->stride_height == 2){
    if(convolution_op->padding_top){
      if(output_width >= (nr + nr / 2) / 2){
        ...
        xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s2_d1_2x4v");
        xnn_x32_packa_in_T_gemm_im2col_s2_d1_2x4v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
          output_height, output_width,
          kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
          convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
          input, input_packed_ptr
        ...
        xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s2_d1_1x4v");
        xnn_x32_packa_in_T_gemm_im2col_s2_d1_1x4v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
          output_height, output_width,
          kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
          convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
          input, input_packed_ptr
...
static enum xnn_status setup_input_T_pruned_igemm_x1v(
  xnn_operator_t convolution_op,
  void* workspace,
  uint32_t log2_input_element_size)
{
  ...
  if(convolution_op->stride_height == 2){
    xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s2_d1_x1v");
  	...
  	xnn_x32_packa_in_T_gemm_im2col_s1_d1_1x1v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
    	output_height, output_width,
    	kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
    	convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
    	convolution_op->input, input_packed_ptr
...
static enum xnn_status setup_input_T_pruned_igemm_x2v(
  xnn_operator_t convolution_op,
  void* workspace,
  uint32_t log2_input_element_size)
{
  ...
  if(convolution_op->stride_height == 2){
    xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s2_d1_x2v");
      ...
      xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_2x2v");
  		xnn_x32_packa_in_T_gemm_im2col_s1_d1_2x2v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
                output_height, output_width,
                kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
                convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
                convolution_op->input, input_packed_ptr
      ...
      xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_1x2v");
      xnn_x32_packa_in_T_gemm_im2col_s1_d1_1x2v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
                  output_height, output_width,
                  kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
                  convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
                  convolution_op->input, input_packed_ptr
...
static enum xnn_status setup_input_T_pruned_igemm_x4v(
  xnn_operator_t convolution_op,
  void* workspace,
  uint32_t log2_input_element_size)
{
...
static enum xnn_status setup_input_T_pruned_igemm_x8v(
  xnn_operator_t convolution_op,
  void* workspace,
  uint32_t log2_input_element_size)
{
  ...
  if(convolution_op->stride_height == 2){
    xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s2_d1_x8v");
  	...
  	if(output_width >= (3 * nr) >> 2){
    xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_8x8v");
		xnn_x32_packa_in_T_gemm_im2col_s1_d1_8x8v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
        	output_height, output_width,
        	kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
        	convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
        	convolution_op->input, input_packed_ptr
      ...
      xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_4x8v");
  		xnn_x32_packa_in_T_gemm_im2col_s1_d1_4x8v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
                output_height, output_width,
                kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
                convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
                convolution_op->input, input_packed_ptr
      ...
      xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_2x8v");
  		xnn_x32_packa_in_T_gemm_im2col_s1_d1_2x8v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
                output_height, output_width,
                kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
                convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
                convolution_op->input, input_packed_ptr
      ...
      xnn_log_debug("using xnn_x32_packa_in_T_gemm_im2col_s1_d1_1x8v");
      xnn_x32_packa_in_T_gemm_im2col_s1_d1_1x8v(batch_size, convolution_op->input_height, convolution_op->input_width, group_input_channels, \
                  output_height, output_width,
                  kernel_height, kernel_width, convolution_op->stride_height, convolution_op->stride_width, \
                  convolution_op->dilation_height, convolution_op->dilation_width, convolution_op->padding_top, convolution_op->padding_left, \
                  convolution_op->input, input_packed_ptr
  ...
  switch (convolution_op->ukernel.type) {
    ...
    case xnn_microkernel_type_input_T_igemm:
      ...
      if(convolution_op->stride_height == 2)
        ...
        if(output_width >= (nr + nr / 2) / 2)
          return setup_input_T_igemm(convolution_op, workspace, log2_input_element_size, convolution_op->context.input_T_gemm.input_T_gemm.gemm.packa_s1_d1_4x4v_method);
        else if(output_width >= (nr / 2 + nr / 4) / 2)
          return setup_input_T_igemm(convolution_op, workspace, log2_input_element_size, convolution_op->context.input_T_gemm.input_T_gemm.gemm.packa_s1_d1_2x4v_method);
        else
          return setup_input_T_igemm(convolution_op, workspace, log2_input_element_size, convolution_op->context.input_T_gemm.input_T_gemm.gemm.packa_s1_d1_1x4v_method);
      }
      else
        XNN_UNREACHABLE;
    default:
...
static enum xnn_status setup_input_T_pruned_convolution2d_nhwc_x1v(
  xnn_operator_t convolution_op,
  enum xnn_operator_type expected_operator_type,
  void* workspace,
  const void* input,
  ...
  uint32_t log2_input_element_size)
{
  ...
  convolution_op->input = input;
  convolution_op->output = output;
  convolution_op->indice = indice;
  const size_t output_width = convolution_op->output_width;
  switch (convolution_op->ukernel.type) {
    case xnn_microkernel_type_input_T_pruned_gemm:
      xnn_log_debug("using setup_input_T_pruned_gemm");
      return setup_input_T_pruned_gemm(convolution_op, log2_input_element_size);
    case xnn_microkernel_type_input_T_pruned_igemm:
...
static enum xnn_status setup_input_T_pruned_convolution2d_nhwc_x2v(
  xnn_operator_t convolution_op,
  enum xnn_operator_type expected_operator_type,
  void* workspace,
  const void* input,
...
static enum xnn_status setup_input_T_pruned_convolution2d_nhwc_x4v(
  xnn_operator_t convolution_op,
  enum xnn_operator_type expected_operator_type,
  void* workspace,
  const void* input,
...
static enum xnn_status setup_input_T_pruned_convolution2d_nhwc_x8v(
  xnn_operator_t convolution_op,
  enum xnn_operator_type expected_operator_type,
  void* workspace,
  const void* input,
...
enum xnn_status xnn_setup_input_T_convolution2d_nhwc_f32(
  xnn_operator_t convolution_op,
  void* workspace,
  const float* input,
  float* output)
{
return setup_input_T_convolution2d_nhwc(
  convolution_op, xnn_operator_type_convolution_nhwc_f32,
  workspace, input, output, /*quantization_params=*/NULL,
  /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT);
}

enum xnn_status xnn_setup_input_T_pruned_convolution2d_nhwc_f32(
  xnn_operator_t convolution_op,
  void* workspace,
  const float* input,
  float* output,
  ...
  const size_t lmul)
{
  switch (lmul) {
    case 1:
      return setup_input_T_pruned_convolution2d_nhwc_x1v(
        convolution_op, xnn_operator_type_convolution_nhwc_f32,
        workspace, input, output, indice,
        /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT);
    case 2:
      return setup_input_T_pruned_convolution2d_nhwc_x2v(
        convolution_op, xnn_operator_type_convolution_nhwc_f32,
        workspace, input, output, indice,
        /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT);
    case 4:
      return setup_input_T_pruned_convolution2d_nhwc_x4v(
        convolution_op, xnn_operator_type_convolution_nhwc_f32,
        workspace, input, output, indice,
        /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT);
    case 8:
      return setup_input_T_pruned_convolution2d_nhwc_x8v(
        convolution_op, xnn_operator_type_convolution_nhwc_f32,
        workspace, input, output, indice,
        /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT);
    default:
