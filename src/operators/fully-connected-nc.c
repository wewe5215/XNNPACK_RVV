...
static enum xnn_status create_input_T_fully_connected_nhwc(
    size_t input_channels,
    size_t output_channels,
    size_t input_stride,
    size_t output_stride,
    ...
    xnn_operator_t* fully_connected_op_out)
{
  ...
  fully_connected_op->input_pixel_stride = input_stride;
  fully_connected_op->output_pixel_stride = output_stride;
  fully_connected_op->kernel = kernel;
  fully_connected_op->bias = bias;
  fully_connected_op->riscv_packa_method = packa_gemm_x4v;
  ...
  const size_t mr = gemm_config->mr;
  fully_connected_op->ukernel.type = xnn_microkernel_type_gemm;
  fully_connected_op->ukernel.input_T_gemm = (struct xnn_ukernel_input_T_gemm) {
    .mr = mr,
    .nr = nr,
    .kr = kr,
    .sr = sr,
  ...
  assert(XNN_MAX_MR >= mr);
  for (size_t i = 0; i < mr; i++) {
    fully_connected_op->ukernel.input_T_gemm.gemm_cases[i] = gemm_ukernels->input_T_gemm[i];
  }
...
enum xnn_status xnn_create_input_T_fully_connected_nc_f32(
  size_t input_channels,
  size_t output_channels,
  size_t input_stride,
  size_t output_stride,
...
return create_input_T_fully_connected_nhwc(
    input_channels, output_channels,
    input_stride, output_stride,
    kernel, bias, flags,
    /*log2_input_element_size=*/XNN_LOG2_SIZEOF_FLOAT,
...
static enum xnn_status reshape_input_T_fully_connected_nc(
  xnn_operator_t fully_connected_op,
  enum xnn_operator_type expected_operator_type,
  size_t batch_size,
  uint32_t log2_input_element_size,
  ...
  pthreadpool_t threadpool)
{
  ...
  size_t input_channels = fully_connected_op->group_input_channels;
  const size_t output_channels = fully_connected_op->group_output_channels;
  const size_t w_stride = (input_channels << log2_filter_element_size);
  uint32_t mr = fully_connected_op->ukernel.input_T_gemm.mr;
  const uint32_t nr = fully_connected_op->ukernel.input_T_gemm.nr;
  struct xnn_hmp_input_T_gemm_ukernel *gemm_cases = fully_connected_op->ukernel.input_T_gemm.gemm_cases;
  ...
  fully_connected_op->context.input_T_gemm.input_T_gemm.gemm = (struct input_T_gemm_context){
      .k_scaled = input_channels << log2_input_element_size,
      .a_stride = fully_connected_op->input_pixel_stride << log2_input_element_size,
      .ga_stride = input_channels << log2_input_element_size,
      .w_stride = w_stride,
      .gw_stride = w_stride * round_up(output_channels, nr),
      .cm_stride = batch_size << log2_output_element_size,
      .cn_stride = nr << log2_output_element_size,
      .gc_stride = output_channels << log2_output_element_size,
      .log2_csize = log2_output_element_size,
      .num_batch_dims = 1,
      .ukernel = gemm_ukernel,
  };
  fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.kernel = fully_connected_op->kernel;
  fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.bias = fully_connected_op->bias;
  fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.packa_method = fully_connected_op->riscv_packa_method;
  memcpy(&fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.params, &fully_connected_op->params, sizeof(fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.params));
  fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.fused_params = &fully_connected_op->context.input_T_gemm.input_T_gemm.gemm.params;

  size_t nc = batch_size;
  const size_t num_threads = pthreadpool_get_threads_count(threadpool);
  if (num_threads > 1) {
    const size_t num_other_tiles = divide_round_up(output_channels, mr);
    const size_t target_tiles_per_thread = 5;
    const size_t max_nc = divide_round_up(batch_size * num_other_tiles, num_threads * target_tiles_per_thread);
    if (max_nc < nc) {
      nc = min(nc, divide_round_up(nc, max_nc * nr) * nr);
    }
  }
  fully_connected_op->compute[0].type = xnn_parallelization_type_2d_tile_2d;
  fully_connected_op->compute[0].task_2d_tile_2d = (pthreadpool_task_2d_tile_2d_t) xnn_compute_input_T_gemm;
...
enum xnn_status xnn_reshape_input_T_fully_connected_nc_f32(
  xnn_operator_t fully_connected_op,
  size_t batch_size,
  pthreadpool_t threadpool)
{
